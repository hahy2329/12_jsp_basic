...
/*
	...
	*/
	...
	// 파일의 저장 경로는 웹서비스의 접근 경로가 아닌 물리적인 실제 경로를 적어주어야 한다.
		...
		//new MultipartRequest(request, saveDirectory, 1024 * 1024 *30, "utf-8", new DefaultFileRenamePolicy());
			//1024 -> 1kb로 인식하고 1024 * 1024 -> 1mb로 인식 1024*1024*1024 -> 1gb
		...
		if(files.hasMoreElements()) { //다음 정보가 있으면 
			...
			String filesystemName = multipartRequest.getFilesystemName(element); //서버에 업로드 '된' 파일명을 반환(리네임 정책을 두들겨 맞고 들어가 있는거)
			String originalFileName = multipartRequest.getOriginalFileName(element); //서버에 업로드 '한' 파일명을 반환(내가 올린거!)
			String contentType= multipartRequest.getContentType(element); //업로드된 파일의 타입을 반환
			long length = multipartRequest.getFile(element).length(); //업로드된 파일의 크기를 반환(long타입)
